...
def choose(paragraphs, select, k):
    ...
    for paragraph in paragraphs:
        if select(paragraph):
            selectable_list.append(paragraph)
    return '' if len(selectable_list) < k + 1 else selectable_list[k]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        ...
        compared_list = [type == reference_words[i] for i, type in enumerate(typed_words)]
        accuracy = sum(compared_list) / len(compared_list) * 100.0
    return accuracy
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        for word in valid_words:
            score_list.append(diff_function(user_word, word, limit))
...
def pawssible_patches(start, goal, limit):
    ...
    if start in goal or goal in start:  # Fill in the condition
        if start == goal:
            return 0
        else:
        ...
        if len(start) == 0 or len(goal) == 0 or start[0] == goal[0]:
            return 0 + abs(len(start) - len(goal))
        else:
            return 1 + abs(len(start) - len(goal))
    else:
