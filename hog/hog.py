...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        ...
        if result == 1:
            flag_one = True
    return total if not flag_one else 1
    # END PROBLEM 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while exit_flag is False:
        ...
        while continue_flag == True:
            if who == 0:
                earned_score = take_turn(num_rolls=strategy0(score0, score1), opponent_score=score1, dice=dice)
                score0 += earned_score
                if score0 >= goal:
                    exit_flag = True
                    break
                continue_flag = extra_turn(score0, score1)
            else:
                earned_score = take_turn(num_rolls=strategy1(score1, score0), opponent_score=score0, dice=dice)
                score1 += earned_score
                if score1 >= goal:
                    exit_flag = True
                    break
